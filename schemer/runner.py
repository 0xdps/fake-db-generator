...
def base_setup(filepath):
    ...
    full_path = (
        filepath if os.path.isabs(filepath) else os.path.join(os.getcwd(), filepath)
    )
    sc = load_schema(full_path)

    engine = create_engine(
        URL.create(**sc.database.model_dump()), echo=sc.database.echo
    )
...
def generate_tables():
    ...
    for table_data in sc.tables:
        tt = Table(
            ...
            *[Index(index.name, *index.columns) for index in table_data.indexes],
        )
            ...
            for index in range(1, table.count + 1):
                ...
                for field in table.fields:
                    results[field.name], commons = faker.get(
                        ...
                        field.args,
                        commons,
                        **extras if field.db_access else {},
                    )
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="populate",
        help="start populating data in tables",
    )
    ...
    parser.add_argument(
        ...
        "--download-schema",
        type=pathlib.Path,
        help="creates the example schema in the provided path",
    )
    ...
    if (args.create or args.populate) and args.file is None:
        parser.error(
            "the following arguments are required: -f/--file : file path of schema"
        )
